...
const AXES_Y = ["Ventes", "Production", "Ingénierie"] as const;
  ...
  return (
    <div className="space-y-8">
      <div ref={r1.ref} className={r1.className}>
        ...
        <p className="text-muted-foreground text-sm mb-6">Carte thermique des faiblesses structurelles du système VOC.</p>
      </div>
        ...
        <div className="consulting-card overflow-x-auto">
          <div className="min-w-[500px]">
            ...
            <div className="grid grid-cols-5 gap-2 mb-2">
              <div />
              {AXES_X.map(x => (
                <div key={x} className="text-center text-xs font-semibold text-muted-foreground uppercase tracking-wide">{x}</div>
              ))}
            </div>
            {/* Rows */}
            {AXES_Y.map(y => (
              <div key={y} className="grid grid-cols-5 gap-2 mb-2">
                <div className="flex items-center text-xs font-semibold text-foreground">{y}</div>
                {AXES_X.map(x => {
                  ...
                  return (
                    <button
                      ...
                      className={`${CELL_CLASSES[cell.level]} heatmap-cell h-16 active:scale-[0.95] transition-transform`}
                    >
      ...
      <div ref={r3.ref} className={r3.className}>
        <div className="bg-primary text-primary-foreground rounded-xl p-8 text-center">
          <p className="text-lg md:text-xl font-medium leading-relaxed max-w-2xl mx-auto">
            ...
            <span className="text-accent font-bold">Le problème est l'absence de système pour les transformer en décisions.</span> »
          </p>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4" onClick={() => setSelected(null)}>
          <div className="bg-card rounded-xl p-6 max-w-lg w-full shadow-2xl animate-reveal" onClick={e => e.stopPropagation()}>
            ...
            <div className="bg-destructive/5 border border-destructive/20 rounded-lg p-4">
              <p className="text-xs uppercase tracking-wide text-destructive font-semibold mb-2">Scénario réel</p>
